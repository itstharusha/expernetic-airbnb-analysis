...
# --- PAGE CONFIG ---
st.set_page_config(
    page_title="Barcelona Airbnb Intelligence",
    page_icon="🏠",
    layout="wide"
)
...
with tab1:
    ...
    with col1:
        st.markdown(f'<div class="metric-card"><div class="metric-value">{len(df_filtered):,}</div><div class="metric-label">Total Listings</div></div>', unsafe_allow_html=True)
    with col2:
        median_price = df_filtered['price'].median()
        st.markdown(f'<div class="metric-card"><div class="metric-value">€{median_price:.2f}</div><div class="metric-label">Median Price</div></div>', unsafe_allow_html=True)
    with col3:
        avg_rating = df_filtered['review_scores_rating'].mean()
        st.markdown(f'<div class="metric-card"><div class="metric-value">{avg_rating:.2f}</div><div class="metric-label">Avg Rating</div></div>', unsafe_allow_html=True)
    with col4:
        active_pct = (len(df_filtered[df_filtered['demand_segment'] == 'active']) / len(df_filtered) * 100) if len(df_filtered) > 0 else 0
        st.markdown(f'<div class="metric-card"><div class="metric-value">{active_pct:.1f}%</div><div class="metric-label">% Active</div></div>', unsafe_allow_html=True)
    ...
    with col_chart2:
        ...
        fig, ax = plt.subplots(figsize=(8, 4))
        ax.pie(rt_counts.values, labels=rt_counts.index, autopct='%1.1f%%', startangle=90, colors=sns.color_palette("pastel"))
        ax.axis('equal')
        st.pyplot(fig)
...
with tab2:
    ...
    with col1:
        ...
        ng_price = df_filtered.groupby('neighbourhood_group')['price'].median().sort_values(ascending=False)
    ...
    with col2:
        ...
        ng_stats = df_filtered.groupby('neighbourhood_group').agg(
            Count=('listing_id', 'count'),
            Median_Price=('price', 'median'),
            Avg_Rating=('review_scores_rating', 'mean')
        ).sort_values('Count', ascending=False)
        st.dataframe(ng_stats.style.format({'Median_Price': '€{:.2f}', 'Avg_Rating': '{:.2f}'}), use_container_width=True)
    ...
    with col_t1:
        ...
        st.dataframe(n_stats.sort_values('Median_Price', ascending=False).head(10).style.format({'Median_Price': '€{:.2f}'}))
    with col_t2:
        st.markdown("**Top 10 Least Expensive**")
        st.dataframe(n_stats.sort_values('Median_Price', ascending=True).head(10).style.format({'Median_Price': '€{:.2f}'}))
...
with tab3:
    ...
    with col1:
        ...
        st.dataframe(sh_stats.style.format({'Median_Price': '€{:.2f}', 'Avg_Rating': '{:.2f}', 'Median_Reviews': '{:.0f}'}))
    ...
    with col2:
        ...
        ax.pie([single, multi, commercial], labels=['Single Listing', 'Multi (2-9)', 'Commercial (10+)'], autopct='%1.1f%%', colors=sns.color_palette("Set2"))
        ax.axis('equal')
        st.pyplot(fig)
    ...
    sns.barplot(x=['Top 1 Host', 'Top 10 Hosts', 'Top 100 Hosts', 'Top 500 Hosts'], y=[top1, top10, top100, top500], palette='magma', ax=ax)
                ...
                with st.spinner("Analyzing listing and querying LLM..."):
                    try:
                        ...
                        if raw_df.empty:
                            ...
                            c2.metric("Predicted Fair Price", f"€{pred_price:.2f}", f"€{(pred_price - current_price):.2f}")
                            ...
                            if groq_client:
                                ...
                                try:
                                    response = groq_client.chat.completions.create(
                                        messages=[
                                            {"role": "system", "content": "You are a professional, data-driven Airbnb pricing consultant. Be direct, insightful, and actionable."},
                                    ...
                                    st.markdown(f'<div class="ai-recommendation">{rec}</div>', unsafe_allow_html=True)
                                except Exception as e:
                                    st.error(f"Failed to generate LLM recommendation: {e}")
                            else:
                                st.warning("Groq API key not configured. Cannot generate AI recommendation.")
                    except Exception as e:
...
with tab5:
    ...
    if st.button("Regenerate Briefings (Simulated)"):
        st.info("In a full production environment, this would trigger a pipeline to query Groq with the latest data warehouse state.")
