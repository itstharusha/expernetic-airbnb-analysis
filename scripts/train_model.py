...
def train_model(args: argparse.Namespace) -> None:
    ...
    feature_cols = [
        ...
        "hosts_time_as_host_years", "is_superhost", "has_rating", "beds_per_person",
    ] + list(neighbourhood_dummies.columns)

    X = df[feature_cols].astype(float)
    y = df["log_price"]

    logger.info(f"Training XGBoost (seed={args.seed}, estimators={args.n_estimators}, depth={args.max_depth})...")
    model = XGBRegressor(n_estimators=args.n_estimators, max_depth=args.max_depth,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--db-path", default="data/warehouse.duckdb", help="Path to DuckDB warehouse.")
